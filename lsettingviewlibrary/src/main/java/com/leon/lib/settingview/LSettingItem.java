...
public class LSettingItem extends RelativeLayout {
    ...
    public LSettingItem(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mRightIcon_check.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                mOnLSettingItemClick.click(isChecked, LSettingItem.this);
            }
        });
    }
    ...
    public void getCustomStyle(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.LSettingView_leftText) {
                ...
                mTextSize = (int) a.getDimension(attr, 20);
            ...
            }else if((attr == R.styleable.LSettingView_rightTextGravity)){
                //设置右边文字显示位置
                int textGravity= a.getInt(attr, 1);
                if(textGravity==0){
                    //左边
                    mTvRightText.setGravity(Gravity.LEFT|Gravity.CENTER_VERTICAL);
                }else {
                    mTvRightText.setGravity(Gravity.RIGHT|Gravity.CENTER_VERTICAL);
                }
            }
        }
        a.recycle();
        ...
        switch (mRightStyle) {
            ...
            case 1:
                if (null != mOnLSettingItemClick) {
                    mOnLSettingItemClick.click(mChecked,LSettingItem.this);
                }
                break;
            case 2:
                //选择框切换选中状态
    ...
    public void setRightTextBackageDrawable(Drawable drawable){
        mTvRightText.setBackgroundDrawable(drawable);
    }
    /**
     * 获取右侧文字
     */
