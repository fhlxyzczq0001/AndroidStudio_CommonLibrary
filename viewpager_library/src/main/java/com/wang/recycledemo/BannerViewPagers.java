...
public class BannerViewPagers extends LinearLayout {
    ...
    public BannerViewPagers(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        initView(context);
    }

    public BannerViewPagers(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param mViewList
     */
     public void setBannerViewPager(final List<Banner_ViewPager_Bean> mViewList){
        if (null == mViewList || mViewList.size() == 0) {
            return;
        }
        this.mViewList = mViewList;
        BannerPagerAdapter bannerPagerAdapter=new BannerPagerAdapter(context, mViewList, configItemListener, new EventClick() {
            @Override
