...
public class Slider extends CustomView {
	...
	protected void setAttributes(AttributeSet attrs) {
		...
		maxSize = attrs.getAttributeIntValue(MATERIALDESIGNXML, "maxSize", 100);// maxSize > min
	...
	protected void onDraw(Canvas canvas) {
		...
		if (value == min) {
			...
			float division = (ball.xFin - ball.xIni) / (maxSize - min);
	...
	private void setValueInRunnable(final int value,final boolean inRunnable) {
		if(placedBall == false && inRunnable == true)
			...
			this.value = value;
			float division = (ball.xFin - ball.xIni) / maxSize;
			ViewHelper.setX(ball,value*division + getHeight()/2 - ball.getWidth()/2);
			ball.changeBackground();
		}
	}
